...
export class AccessoriesRevokeComponent {
  ...
  saveAndFetch() {
    ...
    if (lostNotReceivedChecked) {
      const var1 = this.accessoriesDetails.deviceLogId;
      ...
      this.deviceLogService.updateRecievedBy(var1, var2, var3).subscribe(
        (response) => {
          ...
          if (response && response.receivedBy) {
            ...
            this.accessoriesDetails.submittedByDate = `${recievedDate}`;
            this.accessoriesDetails.actionName = `${actionName}`;
          }
        },
        (error) => {
    ...
    if (this.newComment) {
      const commentDto = {
        ...
        deviceId: this.accessoriesDetails.deviceId,
        deviceLogId: this.accessoriesDetails.deviceLogId
      };
  ...
  resetRadioButtons() {
    const radioButtons = document.getElementsByName("accessoryReceived");
